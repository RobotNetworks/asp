...
import { SessionError } from "../src/server/store/sessions.js";
import { openDatabase } from "../src/server/store/sqlite.js";
import { SqliteSessionStore } from "../src/server/store/sqlite-sessions.js";
...
describe("SqliteSessionStore/create", () => {
  it("creates a session with creator as joined participant", () => {
    const store = make();
  ...
  it("adds invitees as invited participants", () => {
    const store = make();
  ...
  it("emits session.message and sets messageSequence for initialMessage", () => {
    const store = make();
    const { session, messageSequence } = store.create({
      creator: "@alice.bot",
  ...
  it("ends the session immediately when endAfterSend is true", () => {
    const store = make();
    const { session } = store.create({
      creator: "@alice.bot",
...
describe("SqliteSessionStore/join", () => {
...
describe("SqliteSessionStore/invite", () => {
...
describe("SqliteSessionStore/send", () => {
...
describe("SqliteSessionStore/leave", () => {
...
describe("SqliteSessionStore/end", () => {
...
describe("SqliteSessionStore/reopen", () => {
...
describe("SqliteSessionStore/getEvents visibility", () => {
...
describe("SqliteSessionStore/subscribe", () => {
  it("calls observer when an event is pushed", () => {
    const store = make();
    const received: string[] = [];
    store.subscribe((event) => received.push(event.type));
    const { session } = store.create({ creator: "@alice.bot" });
    store.send(session.id, "@alice.bot", "hi");
    assert.ok(received.includes("session.message"));
  });

  it("unsubscribe stops delivery", () => {
    const store = make();
    const received: string[] = [];
    const unsub = store.subscribe((event) => received.push(event.type));
    const { session } = store.create({ creator: "@alice.bot" });
    unsub();
    store.send(session.id, "@alice.bot", "after unsub");
    assert.ok(!received.includes("session.message"));
  });
});
