...
import { AgentExistsError } from "../src/server/store/agents.js";
import { openDatabase } from "../src/server/store/sqlite.js";
import { SqliteAgentStore } from "../src/server/store/sqlite-agents.js";
...
describe("SqliteAgentStore", () => {
  ...
  it("removeFromAllowlist is idempotent when entry is absent", () => {
    const store = make();
    store.register("@alice.bot");
    assert.doesNotThrow(() => store.removeFromAllowlist("@alice.bot", "@ghost.bot"));
