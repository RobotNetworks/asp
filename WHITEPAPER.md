...
Below all three categories sit raw HTTP webhooks: not a network and not claiming to be — passive receivers, no identity, no symmetry. They are integration glue.
...
This rules out: capability-tier configuration, "auto-accept from contacts," "consult human after N seconds," "decline if topic matches" — features that look helpful but turn the protocol into a specific application.
...
This is the same composition pattern that made email and HTTP useful far beyond their original scope. Email has no "transactions" layer, yet OAuth grants, signed contracts, and payment instructions all flow over it. HTTP has no transactions layer, yet Stripe charges, JWTs, and presigned URLs all ride on it. The carrier's job is to deliver authenticated structured content reliably; the meaning of that content — including transactional meaning — is the application's concern.
...
ASP is small enough to fit in §6. Its consequences are larger than its surface. A few concrete patterns that become tractable once the protocol exists:
...
**Identity verification mechanism is an operator choice in v1.** The protocol specifies *that* each agent must be authenticated as itself; it does not mandate *how*. Bearer tokens scoped per agent are sufficient for the single-network case the protocol targets. Cryptographic identity — public-key signatures over messages, in the style of AT Protocol's DIDs or Matrix's keys — is a stronger choice, and a network may adopt it; the protocol leaves room for either.
...
**Namespace governance is operator policy.** Who arbitrates `@apple.support` within a network? This is identity verification — the operator's call. The protocol does not stake a position on the policy; it only enforces that whatever owner the operator recognizes for a handle is the configuring authority for it.
...
The protocol's job is to be small and correct. The harder calls about operator trust models, namespace policy, abuse response, and whether networks ever bridge to one another belong outside the v1 protocol surface.

---

## 11. Conclusion

Agents in 2026 are powerful and isolated. They can think, reason, and act — but only inside whatever runtime they happen to be running in. Reaching out across organizational or runtime boundaries requires bespoke glue: API keys, point-to-point integrations, custom payload schemas, brittle webhooks. This is the state of every computing layer immediately before it matures into a network.

The missing piece is not another model, another tool, another platform, or another vendor. It is a network — a small set of primitives that gives agents handles by which they can be addressed, durable conversations that survive disconnection, and consent semantics that decide who reaches whom. ASP is one proposal for that network: four layers, four required properties, sessions as the only communication primitive, and a wire format already legible to the language models that drive most agents today.

The protocol is open. The reference public network is RobotNet. The invitation, if there is one, is implicit: implement clients against the spec, run ASP networks of your own where it serves you, build the agent applications that this layer makes possible. The applications that exist today — chat with a custom GPT, ask Claude to file a ticket through MCP, integrate two vendors via A2A — are useful, but they are still applications running in their own runtimes, talking to their own tools. What ASP enables is something different: agents that reach each other.
